...
local function install_string_method_compatibility()
	if type(BeardLibInstallStringMethodCompatibility) == "function" then
		return BeardLibInstallStringMethodCompatibility()
	end
...
install_string_method_compatibility()
...
elseif F == "menuarmourbase" then
	local apply_cosmetics = MenuArmourBase._apply_cosmetics
	local update_character_visuals = MenuArmourBase.update_character_visuals
	local unpack_result = unpack or table.unpack
	local safe_inspect_depth = 0
	local saved_inspect

	local function begin_safe_inspect()
		install_string_method_compatibility()

		if safe_inspect_depth == 0 then
			saved_inspect = inspect
			inspect = tostring
		end

		safe_inspect_depth = safe_inspect_depth + 1

		return true
	end

	local function end_safe_inspect()
		if safe_inspect_depth <= 0 then
			return
		end
		...
		safe_inspect_depth = safe_inspect_depth - 1
		if safe_inspect_depth == 0 then
			inspect = saved_inspect
			saved_inspect = nil
		end
	end

	local function call_with_safe_inspect(func, ...)
		local changed_inspect = begin_safe_inspect()
		local result = {pcall(func, ...)}
		if changed_inspect then
			end_safe_inspect()
		end
	...
	function MenuArmourBase:_apply_cosmetics(...)
		return call_with_safe_inspect(apply_cosmetics, self, ...)
	end

	function MenuArmourBase:update_character_visuals(...)
		return call_with_safe_inspect(update_character_visuals, self, ...)
	end

	Hooks:PreHook(MenuArmourBase, "_apply_cosmetics", "BeardLibSafeInspectApplyCosmeticsPre", function()
		begin_safe_inspect()
	end)

	Hooks:PostHook(MenuArmourBase, "_apply_cosmetics", "BeardLibSafeInspectApplyCosmeticsPost", function()
		end_safe_inspect()
	end)
