local function open_menu(menu)
    local clss = BeardLib.Menus[menu]
    if clss then
        clss:SetEnabled(true)
    else
        BeardLib:log("[Callback Error] Menu %s was not found", tostring(menu))
    end
end

local function menu_callback(menu)
    return function()
        open_menu(menu)
    end
end
...
Hooks:Add("MenuManagerInitialize", "BeardLibModsManagerButtons", function()
    if BeardLib:GetGame() ~= "raid" then
        ...
        if not node:item("BeardLibMenu") then
            MenuCallbackHandler.BeardLibMenu = menu_callback("Mods")
            ...
            MenuCallbackHandler.BeardLibAchievementsMenu = menu_callback("Achievement")
            MenuHelperPlus:AddButton({
        ...
        RaidMenuHelper:MakeClbk("BeardLibMenu", menu_callback("Mods"))
        RaidMenuHelper:MakeClbk("BeardLibAchievementsMenu", menu_callback("Achievement"))
