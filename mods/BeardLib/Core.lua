...
function BeardLib:InstallStringMethodCompatibility()
	if _G.BeardLibStringMethodCompatibilityInstalled then
		return true
	end
	...
	local string_metatable = type(debug.getmetatable) == "function" and debug.getmetatable("") or getmetatable("")
	if type(string_metatable) ~= "table" then
		string_metatable = {}
	end

	local previous_index = string_metatable.__index
	if previous_index ~= string then
		string_metatable.__index = function(value, key)
			local string_method = string[key]
			if string_method ~= nil then
				return string_method
			end

			if type(previous_index) == "function" then
				return previous_index(value, key)
	...
	_G.BeardLibStringMethodCompatibilityInstalled = true

	return true
end

_G.BeardLibInstallStringMethodCompatibility = function()
	return BeardLib:InstallStringMethodCompatibility()
end

BeardLib:InstallStringMethodCompatibility()
...
function BeardLib:Init()
	...
	local function load_configured_module(name, config)
		if config and BeardLib.modules[name] then
	...
	load_configured_module("Options", self._config.load_modules.Options)
	...
	for name, config in pairs(self._config.load_modules) do
		if name ~= "Options" then
			load_configured_module(name, config)
		end
	end
...
function BeardLib:InitBLTHookOwner(hook_data)
	if not hook_data or not hook_data.mod or not hook_data.mod.GetPath then
		return
	end

	local base = self.Frameworks and self.Frameworks.Base
	if not base or not base._loaded_mods then
		return
	end

	local mod_path = hook_data.mod:GetPath()
	if not mod_path or mod_path == self.ModPath then
		return
	end

	local normalized = Path:Normalize(mod_path)
	if string.ends(normalized, "/") then
		normalized = string.sub(normalized, 1, -2)
	end

	local folder_name = Path:GetFileName(normalized)
	if not folder_name then
		return
	end

	local mod = base._loaded_mods[folder_name]
	local main_file = Path:Combine(mod_path, base.main_file_name)
	if not mod and FileIO:Exists(main_file) then
		base:LoadMod(folder_name, mod_path, main_file)
		mod = base._loaded_mods[folder_name]
	end

	local config = mod and mod._config
	if not mod or not config or mod._disabled or mod.modules_initialized or not config.global_key or not config.core_class then
		return
	end

	mod:PreInitModules(base.auto_init_modules)
end

function BeardLib:InstallBLTHookOwnerInit()
	if self._blt_hook_owner_init_installed then
		return
	end

	self._blt_hook_owner_init_installed = true
	if BLT and BLT.RunHookFile then
		local run_hook_file = BLT.RunHookFile

		function BLT:RunHookFile(path, hook_data)
			BeardLib:InitBLTHookOwner(hook_data)
			return run_hook_file(self, path, hook_data)
		end
	end

	if Hooks and Hooks.AddHook then
		local add_hook = Hooks.AddHook

		function Hooks:AddHook(key, id, func)
			local owner = ModInstance
			if type(func) == "function" and owner and owner.GetPath then
				local original_func = func

				func = function(...)
					BeardLib:InitBLTHookOwner({mod = owner})
					return original_func(...)
				end
			end

			return add_hook(self, key, id, func)
		end
	end
end
...
BeardLib:Init()
BeardLib:InstallBLTHookOwnerInit()
