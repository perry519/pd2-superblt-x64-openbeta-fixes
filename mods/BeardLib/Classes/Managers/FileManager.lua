...
local function file_key(value)
	if type(value) == "string" then
		return Idstring(value):key()
	end
...
function BeardLibFileManager:Get(ext, path)
	local k_ext = file_key(ext)
	local k_path = file_key(path)
	return k_ext and k_path and Global.fm.added_files[k_ext] and Global.fm.added_files[k_ext][k_path] or nil
end
...
function BeardLibFileManager:HasScriptMod(ext, path)
	local k_ext = file_key(ext)
	local k_path = file_key(path)
	return k_ext and k_path and self.modded_files[k_ext] and self.modded_files[k_ext][k_path]
end
